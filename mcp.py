...
def create_session(session_id: Optional[str] = None, protocol_version: str = "2024-11-05") -> str:
...
# MCP Tool Functions
def mcp_run_container(image: str, command: list = None, environment: dict = None, 
                     volumes: dict = None, pull_policy: str = "missing") -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "content": [
                {
                    "type": "text", 
                    "text": f"Container execution failed: {str(e)}"
                }
            ],
            "isError": True
        }

def mcp_create_volume(name: str, driver: str = "local", driver_opts: dict = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"Volume creation failed: {str(e)}"
                }
            ],
            "isError": True
        }
    ...
    try:
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"Docker health check failed: {str(e)}"
                }
            ],
            "isError": True
        }
    ...
    try:
            ...
            "content": [
                {
                    ...
                    "text": f"Found {len(container_list)} containers:\n" + 
                           "\n".join([f"- {c['name']} ({c['id']}): {c['status']}" for c in container_list])
                }
            ]
        }
    except Exception as e:
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"Container listing failed: {str(e)}"
                }
            ],
            "isError": True
        }
    ...
    try:
            ...
            "content": [
                {
                    ...
                    "text": f"Found {len(image_list)} images:\n" + 
                           "\n".join([f"- {', '.join(img['tags'])} ({img['id']}): {img['size']}" for img in image_list])
                }
            ]
        }
    except Exception as e:
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"Image listing failed: {str(e)}"
                }
            ],
            "isError": True
        }
...
def create_mcp_app():
    ...
    async def get_mcp(request: Request):
        ...
        if "text/event-stream" not in accept_header:
            return JSONResponse(
                content={"error": "SSE endpoint requires Accept: text/event-stream header"},
                status_code=406,
                headers={"Access-Control-Allow-Origin": "*"}
            )
